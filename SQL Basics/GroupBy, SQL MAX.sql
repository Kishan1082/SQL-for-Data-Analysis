...
select max(PaymentMethod) [Maximum Pay Method] from dbo.sales

-- maximum quantity sold for each productID
select productid, max(quantity) [Maximum Quantit] from dbo.Sales
group by ProductID

select * from dbo.Sales
--Maximum toatl amount for all distinct dates in saledate column
